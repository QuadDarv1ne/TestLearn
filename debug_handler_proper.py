...
# We need to create a mock request that matches what the handler expects
class MockRequest:
    def __init__(self, limiter_instance):
        self.app = type('MockApp', (), {'state': type('MockState', (), {'limiter': limiter_instance})()})()
        self.client = type('MockClient', (), {'host': 'testclient'})()

try:
    request = MockRequest(limiter)
    ...
    print(f"Handler response body: {response.body}")
    print(f"Handler response headers: {dict(response.headers)}")
